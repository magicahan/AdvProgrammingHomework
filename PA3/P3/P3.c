...
void Bin_array_init(Bin_array* bin_array, double** array_list, int* size_array, int max, int min, int num_bin){
	double* bins = malloc(sizeof(double)*(num_bin+1));
	double interval = (max - min)/num_bin;
	int*** indicies = malloc(sizeof(int**)*NUM_ARRAY);

	bin_array->arrays=array_list;

	bins[0] = min;
	/*firstly construct the bins*/
	for(int i=0;i<num_bin;i++){
		if(i!=num_bin-1){
			bins[i+1]=min+interval*(i+1);
		}
		else{
			bins[i+1] = max;
		}
	...
	bin_array->bins=bins;
	/*now constuct the bounding indicies*/
	for(int i=0;i<NUM_ARRAY;i++){
		int index_counter = 0;
		indicies[i] = malloc(sizeof(int*)*num_bin);
		for(int k = 0; k < num_bin; k++){
			indicies[i][k] = malloc(sizeof(int)*2);
		}
		indicies[i][0][0] = 0;
		for(int j = 1; j < size_array[i]; j++){
			if(index_counter<num_bin){
				if(array_list[i][j]>bins[index_counter+1]){
					indicies[i][index_counter][1] = j;
					index_counter+=1;
					if(index_counter<num_bin){
					indicies[i][index_counter]=malloc(sizeof(int)*2);
					indicies[i][index_counter][0] = j;
				}
				}
				else{
					indicies[i][index_counter][1] = j;
		...
		if(index_counter<num_bin){
			for(int m = index_counter+1; m < num_bin; m++){
				indicies[i][m][0] = size_array[i]-1;
				indicies[i][m][1] = size_array[i]-1;
			}
		}
		indicies[i][num_bin-1][1]=size_array[i]-1;
	}
	bin_array->indicies=indicies;
...
size_t binary_search_index(double* array, double target, int start_index, int end_index){
	size_t index, max_index, min_index;
	double array_num;
...
size_t* search_kernel_bin(double target, Bin_array* bin_array, int bin_size){
	size_t* array_index;
	int index;
	...
	for(int i = 0; i<bin_size; i++){
		if((target>=bin_array->bins[i])&&(target<=bin_array->bins[i+1])){
			index = i;
			break;
		}
	}
	...
	for(int i = 0; i<NUM_ARRAY; i++){
		array_index[i] = binary_search_index(bin_array->arrays[i], target, bin_array->indicies[i][index][0], bin_array->indicies[i][index][1]);
	}
	return array_index;
}
...
void lookup_kernel(int time){
	...
	size_t* array_index;
	double* max_min = readin(array_list, size_array);
	...
	Bin_array* bin_array=malloc(sizeof(Bin_array));
	Bin_array_init(bin_array, array_list, size_array, max, min, 300);

	float original_time, union_time, augment_time, bin_time;
	original_time = 0;
	union_time = 0;
	augment_time = 0;
	...
	for(int i = 0; i < time; i++){
		target = rand()*20;
		clock_t t1 = clock();
		array_index = search_kernel_bin(target, bin_array, 300);
		t1 = clock() - t1;
		bin_time += (float)t1/CLOCKS_PER_SEC;
	}
...
int main(int argc, char** argv){
	...
	Array_original* array_original = malloc(sizeof(Array_original));
	Array_original_init(array_original, array_list);
	size_t* array_index = search_kernel_original(target, array_original, size_array);

	// printf("passed\n");
	Bin_array* bin_array=malloc(sizeof(Bin_array));
	Bin_array_init(bin_array, array_list, size_array, max, min, 2);
	// printf("passed2\n");
	size_t* array_index2 = search_kernel_bin(target, bin_array, 2);

	// for(int i =0; i<4;i++){
	// 	printf("index is %lu %lu\n", array_index[i], array_index2[i]);
	// }
	return 0;
}